...
sequenceDiagram
    ...
    participant P as DeleteByIdMapperProvider
    participant DB as Database

    S->>M: deleteById(id)
    M->>P: 生成SQL
    Note over P: UPDATE table<br/>SET deleted = DELETE_TIME_EXPR<br/>（LEFT(DATE_FORMAT(NOW(3), '%Y%m%d%H%i%s%f'), 17)，17位毫秒时间戳）<br/>WHERE id = #{id} AND deleted = 0
    P-->>M: 返回SQL
    M->>DB: 执行UPDATE
    DB-->>M: 影响行数

    S->>M: selectByEntity(entity)
    M->>P: buildWhereClause
    Note over P: 自动追加 AND deleted = 0
    P-->>M: 返回SQL
    M->>DB: 执行SELECT
    DB-->>M: 过滤已删除数据
```
...
- [场景1] Given 调用 deleteById(id), When 执行 SQL, Then 生成 UPDATE SET deleted = 毫秒级时间戳（yyyyMMddHHmmssSSS，17位，BIGINT 安全）而非 DELETE
...
- [异常场景] Given deleted 字段值使用毫秒级时间戳, When 并发删除同一记录, Then 每条记录的 deleted 值唯一，可区分不同删除操作
