...
- [场景4] Given 使用 PageQuery.page(pageable, param, function), When 执行查询, Then 从 Pageable 接口获取分页参数，自动管理分页生命周期
...
- `sh-mybatis/src/main/java/com/wkclz/mybatis/helper/PageQuery.java` (PageHelper分页工具，自动管理生命周期，支持 BaseEntity 和 Pageable 接口)
...
- `sh-core/src/main/java/com/wkclz/core/base/Pageable.java` (分页接口，定义分页参数获取方法)
