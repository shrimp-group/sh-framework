...
sequenceDiagram
    ...
    participant SPI as UserNameProvider
    participant B as BaseEntity

    C->>A: 响应体写入前拦截
    A->>R: 递归收集 BaseEntity<br/>最大深度8层
    Note over A: 穿透 R / PageData / List / Map

    R-->>A: 收集到所有 BaseEntity 实例
    A->>A: 提取所有 createBy / updateBy 的 userCode

    A->>SPI: getNamesByUserCodes(userCodeSet)
    Note over SPI: DCL懒加载<br/>仅查找一次并缓存
    SPI-->>A: userCode → name 映射

    A->>B: 设置 createByName / updateByName
    B-->>A: 填充完成
    A-->>C: 继续写入响应体
```
